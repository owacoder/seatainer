 ...
 * When the SHA256 IO device is closed, the hash will be written to the underlying IO device. If the write fails, the close fails as well.
 *
 ...
 * When the SHA256 IO device is closed, nothing will be written to the underlying IO device.
 *
...
#ifdef __cplusplus
class Sha256IO : public IODevice {
    IODevice *d;
...
public:
    Sha256IO() : d(NULL) {}
    Sha256IO(IODevice &d, const char *mode = "wb") : d(NULL) {tryOpen(open(d, mode));}
    ...
    int open(IODevice &dev, const char *mode = "wb") {
        ...
        m_io = io_open_sha256(dev.underlyingDevice(), mode);
